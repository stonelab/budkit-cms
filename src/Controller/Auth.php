...
class Auth extends Controller {
    ...
    final public function signout() {
        ...
        //$this->redirect("/");
        $this->application->dispatcher->redirect("/auth/signin");
    }

    final public function resetPassword(){

        $this->view->setData("title", "Reset Password");
        $this->view->setLayout("auth/reset");
        

    }

    final public function resendVerificationEmail( User $user , $verification = null ){
        ...
        $mail   = array(
            ...
            "link"=> $this->application->uri->externalize("/auth/signin/verify/{$verification}:{$salt}")
        );
    ...
    public function verifyEmail( $key ){
        ...
        $this->application->dispatcher->redirect("/auth/signin", HTTP_FOUND, null);
                ...
                try{
                    ...
                    if($pass){
                        ...
                        if(empty($verified) ) {
                            ...
                            $this->resendVerificationEmail($this->user);
...
//        $this->view->setData("name", "Livingstone");
        $this->view->setLayout("auth/signin");
    }
...
//        $this->view->setData("name", "Livingstone");
        $this->view->setLayout("auth/signup");
        ...
        if($this->application->input->methodIs("post")){
            ...
            if(!$this->user->getCurrentUser()->isAuthenticated()) {
                ...
                if (!$this->user->store($this->application->input->data("post"), true)):
                    $this->response->addAlert("We could not create your account", "error");
                    return false;
                endif;
